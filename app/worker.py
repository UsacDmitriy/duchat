...
async def reminder_worker(bot: Bot, store: ReminderStore, poll_interval: int) -> None:
    ...
    while True:
        try:
            ...
            logger.info("Reminder worker cancelled; exiting loop")
            raise
        except Exception as exc:  # noqa: BLE001 — фиксируем и продолжаем цикл
            logger.exception(
                "Reminder worker iteration failed, retrying in %s seconds: %s",
                poll_interval,
                exc,
            )
